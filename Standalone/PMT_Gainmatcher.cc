...
double fill_energy_histogram(ReducedDataScanner& R, TH1* h, double escale = 1000./38000.) {
    printf("Building energy histogram...\n");
    h->Sumw2();
...
void bg_subtraction_study() {
    ...
    double nbins = 200;
    double emax = 1500;
    double yscale = 1000.0;
    const char* ylbl = "event rate [mHz/keV]";
    
    std::vector<ReducedDataScanner*> Rb;
    std::vector<TH1F*> hBG;
    ExponentialIntegralFitter IIF;
    HistogramSequenceFitter HSF(&IIF);
    
    for(unsigned int i=0; i<3; i++) {
        Rb.push_back(new ReducedDataScanner(false));
        for(int j=0; j<4; j++) Rb.back()->addRun(RunID(1326+2*i,j));
        ...
        intervalList L;
        L.push_back(pair<double,double>( i*3600, i*3600 + dt));
        HSF.addData(hBG.back(),L);
    }
    ...
    intervalList L;
    L.push_back(pair<double,double>( 20*60, 20*60 + dt));
    TH1* hBiBg = HSF.interpolate(L);
