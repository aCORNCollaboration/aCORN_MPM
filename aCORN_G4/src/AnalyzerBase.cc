...
aCORN_G4_Analyzer::aCORN_G4_Analyzer(const std::string& outfname): anaTree(NULL),
outf(new TFile(outfname.c_str(),"RECREATE")), myevt(new MCEvent()) { }

void aCORN_G4_Analyzer::analyzeFileList(const string& flist) {
	ifstream file;
...
aCORN_G4_Analyzer::~aCORN_G4_Analyzer() {
	if(outf) {
		outf->cd();
		anaTree->Write();
		outf->Close();
...
void aCORN_G4_Analyzer::analyzeFile(const string& fname) {
