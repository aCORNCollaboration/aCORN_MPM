...
G4VPhysicalVolume* DetectorConstruction::Construct() {
	...
	G4UserLimits* myGasLimits = new G4UserLimits();
	myGasLimits->SetMaxAllowedStep(1*cm);
	...
	if(sGeometry=="siDet") {
		...
		for(Side s = EAST; s <= WEST; ++s) {
			...
			trap.trap_win_log[s]->SetUserLimits(mySolidLimits);
			dets[s].mwpc.container_log->SetUserLimits(myGasLimits);
			dets[s].mwpc.winIn_log->SetUserLimits(mySolidLimits);
			dets[s].mwpc.winOut_log->SetUserLimits(mySolidLimits);
			dets[s].mwpc.kevStrip_log->SetUserLimits(mySolidLimits);
			dets[s].scint.container_log->SetUserLimits(mySolidLimits);
		}
